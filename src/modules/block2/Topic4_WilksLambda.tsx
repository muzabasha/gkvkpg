...
export const Topic4_WilksLambda: React.FC<TopicProps> = ({ projectorMode }) => {
  ...
  // Lab parameters
  const [errorSpread, setErrorSpread] = useState<number>(10);
  const [hypSpread, setHypSpread] = useState<number>(8);

  const toggleSection = (id: string) => {
    setOpenSections(prev => ({ ...prev, [id]: !prev[id] }));
  };

  // Compute Wilks' Lambda (E / (E + H))
  const wilksLambda = errorSpread / (errorSpread + hypSpread);

  const chartData = [
    { name: 'Error Var (E)', val: errorSpread, fill: '#3b82f6' },
    { name: 'Hypothesis Var (H)', val: hypSpread, fill: '#10b981' },
    { name: 'Total Var (E+H)', val: errorSpread + hypSpread, fill: '#8b5cf6' }
  ];
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className={`${fontHeading3} m-0 text-blue-600 dark:text-blue-400`}>
                SECTION 1 — Storytelling: The Multi-Cohort Classroom Audits
              </h3>
              <p className="text-xs text-brandDark-400 m-0">Measuring group differences using ratio determinants.</p>
            </div>
          </div>
          ...
          <div className={`p-6 ${fontBody} space-y-4`}>
            <div className="border-l-4 border-primary-500 pl-4 py-1 bg-primary-500/5 rounded-r-xl">
              <p className="italic font-medium text-brandDark-700 dark:text-brandDark-300">
                Suppose you are the Dean of Academics at REVA University. You want to audit if three different classrooms are performing differently on their assessments. You measure **[Math Grades, Attendance]ᵀ** across all cohorts.
              </p>
              <p className="italic font-medium text-brandDark-700 dark:text-brandDark-300 mt-2">
                There are two types of variance in your data: **Within-Group Variance (<MathText math="\\mathbf{E}" />)** (natural differences of students inside the same room) and **Between-Group Variance (<MathText math="\\mathbf{H}" />)** (the difference between the room averages).
                If the room averages are identical, <MathText math="\\mathbf{H}" /> is zero, and the ratio of error to total is 1. If the classrooms are completely different, <MathText math="\\mathbf{H}" /> is huge, and the ratio collapses to 0. This ratio of determinants is **Wilks' Lambda (<MathText math="\\Lambda" />)**!
              </p>
            </div>
          </div>
        )}
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-brandDark-400 m-0">Anatomy of MANOVA, determinant ratios, and likelihood estimates.</p>
            </div>
          </div>
          ...
          <div className="p-6 space-y-6">
            <div className={`${fontBody} space-y-8`}>
              <div className="space-y-3">
                <span className="text-xs font-bold uppercase tracking-wider text-primary-500 block">A. Wilks' Lambda Criterion</span>
                <p>In Multivariate Analysis of Variance (MANOVA), Wilks' Lambda measures the ratio of the error matrix determinant to the total matrix determinant:</p>
                <Eq n="4.1" math="\Lambda = \frac{|\mathbf{E}|}{|\mathbf{E} + \mathbf{H}|} = \prod_{i=1}^p \frac{1}{1 + \lambda_i}" label="Wilks' Lambda" />
                ...
                <div className="overflow-x-auto rounded-xl border border-brandDark-200 dark:border-brandDark-800">
                  <table className="w-full">
                    ...
                    <tbody className="divide-y divide-brandDark-100 dark:divide-brandDark-800">
                      <Term sym="\mathbf{E}" meaning="Sum of squares and cross-products matrix for Error (Within-groups)." />
                      <Term sym="\mathbf{H}" meaning="Sum of squares and cross-products matrix for Hypothesis (Between-groups)." />
                      <Term sym="\lambda_i" meaning="Eigenvalues of E⁻¹H, representing canonical correlations." />
                    </tbody>
                  </table>
                </div>
              </div>
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-brandDark-400 m-0">Group tasks mapping Wilks' Lambda properties.</p>
            </div>
          </div>
        ...
        {openSections.sec3 && (
          <div className="p-6 space-y-6">
            ...
            <div className="p-5 rounded-xl border border-brandDark-100 dark:border-brandDark-800/80 bg-brandDark-50/50 dark:bg-brandDark-950/10">
              ...
              <p className="text-sm">
                <strong>Objectives:</strong> Establish foundational intuition for Wilks Lambda.
              </p>
              <div className="text-sm space-y-2 mt-2 pl-3 border-l-2 border-brandDark-200 dark:border-brandDark-700">
                ...
                <p><strong>Step 2:</strong> Walk through a real-world scenario demonstrating how Wilks Lambda solves the problem.</p>
              </div>
            </div>
            ...
            <div className="p-5 rounded-xl border border-brandDark-100 dark:border-brandDark-800/80 bg-brandDark-50/50 dark:bg-brandDark-950/10">
              ...
              <p className="text-sm">
                <strong>Objectives:</strong> Guided formulation of Wilks Lambda components.
              </p>
              <p className="text-sm">
            ...
            <div className="p-5 rounded-xl border border-brandDark-100 dark:border-brandDark-800/80 bg-brandDark-50/50 dark:bg-brandDark-950/10">
              ...
              <p className="text-sm">
                <strong>Objectives:</strong> Relate Wilks Lambda theory to practical outputs.
              </p>
              <p className="text-sm">
            ...
            <div className="p-5 rounded-xl border border-brandDark-100 dark:border-brandDark-800/80 bg-brandDark-50/50 dark:bg-brandDark-950/10">
              ...
              <p className="text-sm">
                <strong>Task:</strong> Compute a tiny 2x2 problem related to Wilks Lambda.
              </p>
              <p className="text-sm">
      ...
      <section className="bg-white dark:bg-brandDark-900 border border-brandDark-200 dark:border-brandDark-800 rounded-2xl overflow-hidden shadow-sm">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className={`${fontHeading3} m-0 text-emerald-600 dark:text-emerald-400`}>
                SECTION 4 — Project Based Learning: Multi-variate Academic Audit
              </h3>
              <p className="text-xs text-brandDark-400 m-0">MANOVA implementation guidelines.</p>
            </div>
          </div>
        ...
        {openSections.sec4 && (
          <div className="p-6 text-sm">
            <p>
              Develop a quantitative platform for REVA University that collects assessments across branches. Use Wilks' Lambda to identify departments showing statistically significant changes in multi-dimensional performance.
            </p>
          </div>
        )}
      </section>
      ...
      <section className="bg-white dark:bg-brandDark-900 border border-brandDark-200 dark:border-brandDark-800 rounded-2xl overflow-hidden shadow-sm">
        ...
        {openSections.sec5 && (
          <div className="p-6 space-y-4 text-sm">
            <div>
              <h5 className="font-bold">Q1: What are the bounds of Wilks' Lambda (<MathText math="\\Lambda" />)? What does $\Lambda \approx 0$ mean?</h5>
              <p className="text-brandDark-600 dark:text-brandDark-400">
                <strong>Answer:</strong> Bounded strictly within <MathText math="[0, 1]" />. A value near <MathText math="0" /> indicates huge differences between group averages (highly significant), while <MathText math="1" /> implies no differences.
              </p>
            </div>
          </div>
        )}
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-brandDark-400 m-0">Simulate Wilks' Lambda values based on Within-Group and Between-Group variances.</p>
            </div>
          </div>
          ...
          <div className="p-6 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="p-4 bg-brandDark-50 dark:bg-brandDark-950 rounded-xl space-y-4">
                ...
                <div>
                  <label className="text-xs font-bold block mb-1">Within-Group Error (E): {errorSpread}</label>
                  <input
                    type="range" min="1" max="25" step="1"
                    value={errorSpread} onChange={(e) => setErrorSpread(Number(e.target.value))}
                    className="w-full"
                  />
                </div>
                <div>
                  <label className="text-xs font-bold block mb-1">Between-Group Hyp (H): {hypSpread}</label>
                  <input
                    type="range" min="0" max="25" step="1"
                    value={hypSpread} onChange={(e) => setHypSpread(Number(e.target.value))}
                    className="w-full"
                  />
                </div>
              </div>
              ...
              <div className="md:col-span-3 bg-brandDark-50 dark:bg-brandDark-950 p-4 rounded-xl flex flex-col justify-between">
                <div>
                  <h5 className="font-bold text-xs uppercase text-brandDark-400 mb-2">Wilks' Lambda Metric</h5>
                  <div className="text-center py-4">
                    <span className="block text-xs text-brandDark-400 font-bold uppercase">Computed Wilks' Lambda (Λ)</span>
                    <strong className="text-4xl font-extrabold text-blue-600">{wilksLambda.toFixed(4)}</strong>
                  </div>
                </div>
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-brandDark-400 m-0">Run MANOVA and Wilks' Lambda in R.</p>
            </div>
          </div>
...
{`# Wilks' Lambda MANOVA test in R
set.seed(42)

# Generate three groups of bivariate data
Y1 <- matrix(rnorm(20, mean=5), ncol=2)
Y2 <- matrix(rnorm(20, mean=7), ncol=2)
Y3 <- matrix(rnorm(20, mean=6), ncol=2)

Y <- rbind(Y1, Y2, Y3)
group <- factor(rep(c("G1", "G2", "G3"), each=10))
...
`}
      ...
      <section className="bg-gradient-to-r from-primary-950 to-brandDark-900 border border-primary-900/50 rounded-2xl p-6 text-white text-sm shadow-md">
        ...
        <p className="text-brandDark-200">
          Wilks' Lambda measures the proportion of unexplained variance in multi-group systems. A smaller value indicates stronger group separation, making it the bedrock of multi-outcome hypothesis tests (MANOVA).
        </p>
      </section>
    </div>
  );
...
export default Topic4_WilksLambda;
